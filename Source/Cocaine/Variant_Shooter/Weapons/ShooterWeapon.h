...
struct FWeaponRay
{
	...
	UPROPERTY(EditAnywhere, Category="Firing")
	float RayHitTraceRadius = 5.f;
	UPROPERTY(EditAnywhere, Category="Firing")
	TSubclassOf<UDamageType> HitDamageType;
};
UCLASS(abstract)
class COCAINE_API AShooterWeapon : public AActor
{
	GENERATED_BODY()
...
protected:
	...
	/** Should a weapon use RayCasts for firing */
	UPROPERTY(EditAnywhere, Category="Firing")
...
protected:
	...
	/** Fire a RayCast toward the target location */
	virtual void FireRayCast(const FVector& TargetLocation);
	
	/** Logic that is the same for both Firing types */
